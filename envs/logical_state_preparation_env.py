"""
Copied from https://github.com/remmyzen/rlftqc/blob/main/rlftqc/envs/logical_state_preparation_env.py
"""
...
import sys
from simulators import PauliString, TableauSimulator, CliffordGates
...
class LogicalStatePreparationEnv(environment.Environment):
    ...
    def get_observation(self, tableau):
        """ Extract the check matrix, i.e., stabilizers for the observation of the RL agent.
        This is just bad naming at this point. It should have been `get_check_matrix`.
    ...
    def action_matrix(self,
                      params: EnvParams | None = None) -> chex.Array:
    ...
    def canonical_stabilizers(self, check_matrix_inp, sign_inp):
        ...
        Runs Gaussian elimination on the input check matrix to get the canonical stabilizers.
        Again bad naming - should have been `get_canonical_stabilizers`.
    ...
    def is_terminal(self, state: EnvState, params: EnvParams | None) -> bool:
    ...
    def get_obs(self, state: EnvState, params: EnvParams | None = None) -> chex.Array:
        """Applies observation function to state.
        If use_max_reward is true, then returns the canonical stabilizers and signs along with the
        max_diff variable (total length of list is 2*n^2 + 2*n + 1). If use_max_reward is false, then the
        last variable is omitted.
    ...
    def observation_space(self, params: EnvParams | None) -> spaces.Box:
    ...
    def state_space(self, params: EnvParams | None) -> spaces.Dict:
